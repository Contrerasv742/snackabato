...
int PingSensor_Init(void) {
    ...
    TMR4 = 0; // clear TMR4
    T4CON = 0; // clear timer 4 control register

    T4CONbits.TCKPS = 0b111; // TMR4 prescaler 1:256 (3'b111)
    PR4 = 9375-1; // TMR4 Period register value for 60 ms rollover
    
    IFS0bits.T4IF = 0; // clear TMR4 interrupt flag status
    IEC0bits.T4IE = 1; // enable TMR4 interrupts
    IPC4bits.T4IP = 3; // set TMR4 priority
