...
static TargetLSubHSMState_t CurrentState = InitPSubState; // <- change name to match ENUM
...
/**
 * @Function InitTargetLSubHSM(uint8_t Priority)
...
uint8_t InitTargetLSubHSM(void)
{
    ES_Event returnEvent;

    CurrentState = InitPSubState;
    returnEvent = RunTargetLSubHSM(INIT_EVENT);
...
/**
 * @Function RunTargetLSubHSM(ES_Event ThisEvent)
...
ES_Event RunTargetLSubHSM(ES_Event ThisEvent)
{
    uint8_t makeTransition = FALSE; // use to flag transition
    TargetLSubHSMState_t nextState; // <- change type to correct enum
    ...
    if (makeTransition == TRUE) { // making a state transition, send EXIT and ENTRY
        ...
        RunTargetLSubHSM(EXIT_EVENT); // <- rename to your own Run function
        CurrentState = nextState;
        RunTargetLSubHSM(ENTRY_EVENT); // <- rename to your own Run function
    }
